...
# 40 fathom fishing limit
iso40_sf <- st_read("./Inputs/XY40fm_03152021_PointsToLine1.shp")
iso40_sf_utm <- st_transform(iso_sf, crs = 32610)
...
# individuals photographed by OSU, excluding fishermen photos
score_df %>% 
  filter(!(file %in% c(str_subset(score_df$file, ".CBarnhart"),
                     str_subset(score_df$file, ".Josh"))) &
             Source %in% c("SOURCE-GEMMLAB", "SOURCE-WHETLAB")) %>% 
  summarize(ind = n_distinct(ID))
...
Function to calculate confidence interval on scarring prevalence estimates
```{r}
Fun_ConfInt <- function(p,n){
  conf_low <- p - 1.96*sqrt((p * (100-p))/n)
  conf_high <- p + 1.96*sqrt((p * (100-p))/n)
  return(c(conf_low, conf_high))
}
```


### Wall et al. Most likely proportion

The percent of individuals showing “Most likely” entanglement scars in either of the five possible photo types is presented following the Wall et al. (2019) approach. 
...
Ent_wallm <- tot_wall_df %>% filter(Entang_num <= 1)
(Nent_wallm <- nrow(Ent_wallm))

# proportion of individuals with at least one body part showing “Most likely” and “Likely”
(Pwallm <- Nent_wallm / Ntot_wall*100)

# confidence interval
Fun_ConfInt(Pwallm, Ntot_wall)
```
Check that the results are similar if the Pwallm was calculated only with 2016-2023 data so as to cover the same time period as Prob
```{r}
tot_wall_df_2016 <- score_bestid_df %>% 
  filter(as.numeric(Year) >= 2016) %>% 
  group_by(ID) %>% 
  summarize(Entang_num = min(Entang_num))
  
# number of individuals considered
(Ntot_wall16 <- nrow(tot_wall_df_2016))

# number of individuals with at least one body part showing “Most likely” and “Likely” entanglement scars
Ent_wallm16 <- tot_wall_df_2016 %>% filter(Entang_num <= 1)
(Nent_wallm16 <- nrow(Ent_wallm16))

# proportion of individuals with at least one body part showing “Most likely” and “Likely”
(Pwallm16 <- Nent_wallm16 / Ntot_wall16*100)

# confidence interval
Fun_ConfInt(Pwallm16, Ntot_wall16)
```
...
# proportion of individuals with at least one body part showing “Most likely” and “Likely”
(Pwall <- Nent_wall / Ntot_wall*100)

# confidence interval
Fun_ConfInt(Pwall, Ntot_wall)
```
...
# proportion of individuals with at least one body part showing “Most likely” and “Likely”
(Prob <- Nent_rob / Ntot_rob*100)

# confidence interval
Fun_ConfInt(Prob, Ntot_rob)
```
...
# did the whale have photos of other body parts or not?
score_bestid_df %>% filter(ID %in% unique(flukedetections$ID))
```
...
```{r}
(Slow <- min(Prob, Pwall, Pwallm))
(Shigh <- max(Prob, Pwall, Pwallm))
```
Compare Shigh and Slow statistically with a Fisher exact test for count data
```{r}
(bin_prev <- rbind(c(Nent_rob, Ntot_rob-Nent_rob), c(Nent_wallm, Ntot_wall-Nent_wallm)))
fisher.test(bin_prev)

# check that results are the same if we focus on 2016-2023
(bin_prev <- rbind(c(Nent_rob, Ntot_rob-Nent_rob), c(Nent_wallm16, Ntot_wall16-Nent_wallm16)))
fisher.test(bin_prev)
...
g <- ggplot() +
    geom_sf(data = iso_sf_utm[iso_sf_utm$g_2020_ %in% c(-50, -100, -500, -1000, -1500), ], col = "grey85", size = 0.01) +
    geom_sf(data = iso_sf_utm[iso_sf_utm$g_2020_ %in% c(-200), ], col = "grey60", size = 0.08) +
    geom_sf(data = coast_sf, col = NA, fill = "grey60", size = 0.08) +
    geom_sf(data = iso40_sf, col = "black") +
    geom_sf_text(data = places, aes(label = name), nudge_x = 0, size = 2.8, col = "grey20") +
    mon_theme +
    xlab("") +
    ylab("") +
...
g
ggsave(g, file = "./Outputs/Fig1.png", width = 90, height = 200, units = "mm", dpi = 600)
```
...
```{r}
# additional analysis of sex effect using the Robbins & Mattila approach
sex_perp_clm_df <- perp_best_df %>% 
  group_by(ID) %>%
  arrange(Perp_Entang_num, .by_group = T) %>% 
  slice_head(n = 1) %>% 
  ungroup() %>%
  mutate(w = ifelse(Perp_Entang_num <= 2, 4-Quality_num, 3)) %>% 
  mutate(w = w / mean(w)) %>% 
  filter(!is.na(Sex)) %>% 
  mutate(Sex = as.factor(Sex),
         Perp_Entang_num = as.factor(Perp_Entang_num))

# ru cumulative link model
m_sex_perp <- clm(Perp_Entang_num ~ Sex, 
                      weights = w,  
                      data = sex_perp_clm_df)

# check data
table(sex_perp_clm_df$Sex, sex_perp_clm_df$Perp_Entang_num)
# assess effect of sex on categorical probability of prior entanglement
car::Anova(m_sex_perp, type = "II")
summary(m_sex_perp)
```
...
# number of whales seen more than once
hb_or_df %>% 
  ddply(., ~ID, function(d){data.frame(nb_sightings = nrow(d))}) %>% 
  filter(nb_sightings > 1) %>% 
  nrow()
